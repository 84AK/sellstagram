...
// ─── 이벤트 아이콘/컬러 ──────────────────────────────────────
const EVENT_META = {
    like: { icon: Heart, color: "#FF6B35", bg: "rgba(255,107,53,0.12)", label: "좋아요" },
    comment: { icon: MessageCircle, color: "#4361EE", bg: "rgba(67,97,238,0.12)", label: "댓글" },
    share: { icon: Share2, color: "#06D6A0", bg: "rgba(6,214,160,0.12)", label: "공유" },
    purchase: { icon: ShoppingBag, color: "#FFC233", bg: "rgba(255,194,51,0.18)", label: "구매" },
};
...
function ResultCard({
    ...
    const summaryText = `📊 마켓 시뮬레이션 결과\n\n` +
        `📝 게시물: ${post.caption.slice(0, 40)}...\n` +
        `⏱️ 시뮬레이션 시간: ${durationMinutes}분 (${durationMinutes}시간 분량)\n\n` +
        `❤️ 좋아요: ${likes}개\n` +
        `💬 댓글: ${comments}개\n` +
        `🔗 공유: ${shares}개\n` +
        `🛍️ 구매: ${purchases}건\n` +
    ...
    return (
        ...
        >
            ...
            <div
                className="px-6 py-5"
                style={{
                    background: "linear-gradient(135deg, #FF6B35 0%, #FFC233 100%)",
                }}
            >
                ...
                <p className="text-white/80 text-sm">{durationMinutes}분 동안 {durationMinutes}시간의 마켓 반응이 시뮬레이션됐어요</p>
            </div>

            {/* 스탯 그리드 */}
            <div className="grid grid-cols-2 gap-3 p-5">
                {[
                    { label: "좋아요", value: likes, icon: "❤️", color: "#FF6B35" },
                    { label: "댓글", value: comments, icon: "💬", color: "#4361EE" },
                    { label: "공유", value: shares, icon: "🔗", color: "#06D6A0" },
                    { label: "구매", value: purchases, icon: "🛍️", color: "#FFC233" },
                ].map(s => (
                    <div
                        key={s.label}
...
// ─── 이벤트 피드 아이템 ─────────────────────────────────────
function EventItem({ event, animate }: { event: SimEvent; animate: boolean }) {
    const meta = EVENT_META[event.type];
    const Icon = meta.icon;

    return (
        <div
            className={`flex items-start gap-3 p-3 rounded-2xl transition-all duration-500 ${animate ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}
        ...
        >
            ...
            <div className="flex-1 min-w-0">
                <div className="flex items-center gap-1.5 mb-0.5">
                    <span className="text-sm font-black" style={{ color: "var(--foreground)" }}>{event.persona.name}</span>
                    <span className="text-[10px]" style={{ color: "var(--foreground-muted)" }}>({event.persona.age}세)</span>
                ...
                {event.comment && (
                    <p className="text-[13px]" style={{ color: "var(--foreground-soft)" }}>{event.comment}</p>
                )}
                {event.type === "purchase" && event.amount && (
                    <p className="text-[13px] font-bold" style={{ color: "#D97706" }}>₩{event.amount.toLocaleString()} 결제 완료</p>
                )}
...
export default function SimulatePage() {
    ...
    // 내 게시물 목록 (post 타입, image 있는 것만)
    const myPosts = posts.filter(p =>
        p.user.handle === user.handle &&
        p.type === "post" &&
        p.content?.image
    );
    ...
    // 이벤트 생성 (simState 또는 selectedPost 변경 시)
    useEffect(() => {
        ...
        const events = generateSimEvents(
            simPost.id,
            simPost.engagementRate,
            simPost.productPrice,
            simState.durationMinutes,
            simState.startedAt,
        );
        setAllEvents(events);
        setVisibleEvents([]);
    ...
    useEffect(() => {
        ...
        const tick = () => {
            const start = new Date(simState.startedAt!).getTime();
            const now = Date.now();
        ...
        try {
            ...
            await fetch("/api/simulate/save-result", {
                ...
                body: JSON.stringify({
                    ...
                    post_id: simPost.id,
                    post_caption: simPost.caption,
                    post_image: simPost.imageUrl,
                    session_started_at: simState.startedAt,
                    duration_minutes: simState.durationMinutes,
                    total_likes: likes,
    ...
    if (!simState.active) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[60vh] gap-6 p-6 max-w-lg mx-auto text-center">
                ...
                <div>
                    <h1 className="text-2xl font-black font-outfit mb-2" style={{ color: "var(--foreground)" }}>
                        마켓 시뮬레이션
                    </h1>
    ...
    if (myPosts.length === 0) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4 p-6 max-w-lg mx-auto text-center">
                ...
                <h2 className="text-xl font-black font-outfit" style={{ color: "var(--foreground)" }}>
                    게시물이 없어요
                </h2>
                <p className="text-sm" style={{ color: "var(--foreground-soft)" }}>
                    시뮬레이션을 시작하려면 먼저 피드에 게시물을 업로드해주세요.
                </p>
                <Link
                    href="/feed"
            ...
            <div className="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p className="text-[11px]" style={{ color: "var(--foreground-muted)" }}>
                        1분 = 1시간 · {simState.durationMinutes}시간 분량
                    </p>
                </div>
            ...
            >
                ...
                <div className="flex justify-between mt-1.5">
                    <span className="text-white/60 text-[10px] font-bold">
                        {Math.floor(elapsedMs / 60000)}분 경과
                    </span>
                    <span className="text-white/60 text-[10px] font-bold">
                        이벤트 {visibleEvents.length}개
                    </span>
                </div>
            </div>
                ...
                >
                    <div className="px-4 py-3 flex items-center justify-between"
                        ...
                        <div className="flex gap-3 text-[10px] font-bold" style={{ color: "var(--foreground-muted)" }}>
                            <span>❤️ {visibleEvents.filter(e => e.type === "like").length}</span>
                            <span>💬 {visibleEvents.filter(e => e.type === "comment").length}</span>
                            <span>🔗 {visibleEvents.filter(e => e.type === "share").length}</span>
                            <span>🛍️ {visibleEvents.filter(e => e.type === "purchase").length}</span>
                        </div>
                    </div>

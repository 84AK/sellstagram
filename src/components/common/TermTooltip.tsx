...
export const TERM_DICT: Record<string, TermDef> = {
    engagement: {
        ...
        example: "예시: 100명이 봤는데 10명이 좋아요·댓글을 달았다면 인게이지먼트율 = 10%",
        tip: "💡 인게이지먼트를 높이려면 질문형 캡션, 명확한 혜택 제시, 공감 가는 스토리가 효과적이에요!",
    },
    roas: {
        term: "ROAS (광고 수익률)",
        emoji: "⚡",
        ...
        detail:
            "ROAS는 'Return On Ad Spend'의 줄임말로, 광고에 쓴 돈 대비 벌어들인 매출의 비율이에요. 예를 들어 ROAS 3x라면 광고비 1만원을 써서 3만원의 매출을 올렸다는 뜻이에요.",
        example: "예시: 광고비 10,000원 → 매출 30,000원 = ROAS 3x",
        tip: "💡 일반적으로 ROAS 2x 이상이면 수익, 1x 미만이면 손실이에요. 목표는 높을수록 좋아요!",
    },
    reach: {
        term: "총 도달 (Reach)",
        emoji: "👥",
        ...
        tip: "💡 도달을 늘리려면 해시태그 활용, 올리는 시간대, 스토리 연동이 중요해요!",
    },
    revenue: {
        term: "매출 (Revenue)",
        emoji: "💰",
        ...
        detail:
            "마케팅 콘텐츠를 통해 구매자가 실제로 지불한 금액의 합계예요. 마케팅 잔고에 반영되어 다음 광고 집행에 활용할 수 있어요.",
    ...
    balance: {
        ...
        tip: "💡 잔고를 잘 활용해서 효율적인 마케팅 전략을 세워보세요!",
    },
    cta: {
        term: "CTA (행동 유도 문구)",
        emoji: "📣",
    ...
    return (
        <div ref={ref} className={`relative inline-flex items-center ${className}`}>
            ...
            {open && (
                <>
                    {/* 모바일: 화면 중앙 모달 */}
                    <div className="fixed inset-0 z-40 flex items-end justify-center p-4 md:hidden"
                        style={{ background: "rgba(0,0,0,0.4)", backdropFilter: "blur(4px)" }}
                        onClick={() => setOpen(false)}>
                        <div
                            className="w-full max-w-sm rounded-3xl p-5 flex flex-col gap-3"
                            style={{ background: "var(--surface)", boxShadow: "0 -4px 32px rgba(0,0,0,0.15)" }}
